...
An enterprise-grade, high-speed **Single-Agent AI Ops Architect** built on the Google **ADK (Agent Development Kit)** framework. This agent directly utilizes **GCP MCP (Model Context Protocol)** toolsets and the **Gemini Enterprise Agent Platform Skill Registry** to monitor, analyze, and troubleshoot Google Cloud resources.

---

## 🚀 Core Features

*   **Direct Tool-Calling Architecture**: Simplifies multi-agent expert routing into a single, high-speed orchestrator agent that directly executes GCP commands—making it 3x faster and cost-effective.
*   **7 Active GCP SRE Services**: Consolidates GKE, Cloud Run, Logging, Monitoring, Network Management, Gemini Cloud Assist, and Error Reporting (53+ tools in total).
*   **Agent Platform Skill Discovery**: Exposes a lightweight semantic search tool to dynamically query specialized SRE handbooks from the Gemini Enterprise Agent Platform Skill Registry.
*   **Unified Execution Logging**: Equipped with robust, bulletproof callbacks that print exact parameters and output results in the terminal whenever any tool or skill is executed.
*   **Clean Package Isolation**: Modularized directory structure hiding all runtime code inside the `app/` subdirectory while proxying entry points at the root.
...
├── agent.py                # Single-Agent definition and callbacks (package-aligned)
├── tools.py                # Procedural GCP MCP toolsets loader (self-contained)
├── skills.py               # Agent Platform semantic skill search tool (package-aligned)
└── agent_platform/         # AgentPlatform SRE administrative scripts
    ├── runtime.py          # Production one-click deployment script
    └── skill_registry.py   # Playbook database management CLI tool
```
...
Create a `.env` file in the root of `google_cloud_ops_agent/` and configure your GCP project variables and target model:
...
# SRE Resources & Skill Registry location
GCP_RESOURCES_LOCATION="us-central1"

# Target LLM Model
GEMINI_MODEL="gemini-3.5-flash"

# Optional: Configure custom MCP endpoints if bypassing public metadata scopes
...
Make sure your virtual environment is activated and ADK dependencies are up to date:
...
To start and interact with the AI Ops Agent, run the commands directly inside the `google_cloud_ops_agent/` directory.
...
### B. Run a Single-Step Query
To execute a single specific instruction and print the response instantly without entering interactive mode:
```bash
adk run --disable_features=JSON_SCHEMA_FOR_FUNC_DECL --enable_features=SKILL_TOOLSET . "GKE node upgrade 스킬이 있는지 검색해줘"
```

### C. Launch the Developer Web UI
To launch the background FastAPI server and open the interactive Web chat panel:
```bash
adk web --disable_features=JSON_SCHEMA_FOR_FUNC_DECL --enable_features=SKILL_TOOLSET .
```
Once the server boots, navigate to the URL printed in the terminal (usually `http://127.0.0.1:8086/dev-ui/`).

---

## 🚀 Vertex AI Agent Engine (Reasoning Engine) Deployment

For production deployments, you can upload this SRE Orchestrator Agent to **Vertex AI Agent Engine (Reasoning Engine)** as a managed, serverless API service.

### 1. Provision the Dedicated Service Account
Create a custom service account and bind the required GCP SRE monitoring viewer roles to it:
...
# Grant general viewer/viewer roles to monitor resources
...
# Grant GCS Storage permissions to read/write artifacts
gcloud projects add-iam-policy-binding ${PROJECT_ID} --member="serviceAccount:${SA_EMAIL}" --role="roles/storage.objectAdmin"

# Grant Service Account User role to your personal deployer account
...
Verify that your `.env` configurations are set up, then run the dedicated `agent_platform/runtime.py` script:

```bash
uv run agent_platform/runtime.py
```

* **How it works (Single-Step Flow)**:
  - **Simultaneous Activation**: The script calls `client.agent_engines.create()` in a single step to deploy your agent package. At the same time, the container automatically consumes the environment variables and instantly activates both the managed Spanner session history database AND the zero-setup managed Memory Bank service!
  
Upon successful deployment, the terminal will print the remote resource name: `projects/{project_number}/locations/us-central1/reasoningEngines/{engine_id}`.

---

## 🛠️ SRE Playbooks Skill Registry Management CLI

For SRE administrators, a dedicated, focused CLI management tool is provided at `agent_platform/skill_registry.py`. This provides a complete, end-to-end administration interface to manage playbooks registered inside the **Agent Platform Skill Registry**.

To run any subcommand, run directly inside the `google_cloud_ops_agent/` directory using `uv run`:

### A. List Registered Skills
List all active playbooks and their unique resource URIs inside your configured project region:
```bash
uv run agent_platform/skill_registry.py list
```

### B. Register/Create a New Skill
Create a new playbook skill directly from a local playbook directory containing `SKILL.md` and playbooks instructions:
```bash
...
### C. Get Skill Details
Query the registry to fetch the detailed spec and description of a registered skill:
```bash
uv run agent_platform/skill_registry.py get --skill-name "[SKILL_ID_OR_RESOURCE_URI]"
```

### D. Delete a Skill
Delete a retired or obsolete playbook skill from the registry:
```bash
uv run agent_platform/skill_registry.py delete --skill-name "[SKILL_ID_OR_RESOURCE_URI]"
```

### E. Retrieve (RAG Search Query)
Perform a semantic search query on your registered playbooks to verify retrieval results:
```bash
uv run agent_platform/skill_registry.py retrieve --query "GKE node upgrade" --top-k 2
```

### F. Bulk Import Skills from a GitHub Repository
Download, parse, and register all SRE playbooks recursively from a public GitHub repository containing `SKILL.md` manuals in a single step:
```bash
...
## ⚙️ Feature Flags & CLI Options Explained

*   **`--disable_features=JSON_SCHEMA_FOR_FUNC_DECL`** *(Mandatory Bypass)*:
    *   **Why it is needed**: ADK's experimental JSON Schema function declaration module has a known bug where it recursively expands infinitely nested Pydantic schemas (especially prevalent in GKE and Kubernetes spec files).
    *   **The effect**: Disabling this option forces ADK to fallback to the standard, highly optimized flat function declaration format. This slashes your prompt size from an exhausted **1.3M tokens** to a featherweight **~10k tokens**, preventing context bloat and accelerating response times.
*   **`--enable_features=SKILL_TOOLSET`** *(Enabler)*:
    *   **Why it is needed**: Explicitly activates the ADK's experimental Enterprise Skill Toolset.
    *   **The effect**: Equips the orchestrator agent with built-in skill loading, listing, and execution playbooks. This allows the agent to semantically search and dynamically run playbooks directly from the remote Agent Platform Skill Registry.

---

## 🧹 Clean Resetting Session History

If your local session database gets too large during rapid testing, clean reset the conversation database:
```bash
# Run from the root google_cloud_ops_agent directory
rm -rf .adk/
```
...
## 🇰🇷 한국어 개발 설명서 (Korean Developer Manual)
